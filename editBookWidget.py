...
class EditBookWidget(QWidget):
    ...
    def __init__(self, selected_book_id) -> None:
        super().__init__()
        self.selected_book_id = selected_book_id
        self.init_ui()
    ...
    def init_ui(self) -> None:
        ...
        try:
            ...
            self.lineEdit_book_name = QLineEdit(str(self.book[1]))
            ...
            self.lineEdit_author = QLineEdit(str(self.book[2]))
            ...
            self.lineEdit_number_of_pages = QLineEdit(str(self.book[3]))
            ...
            self.lineEdit_publisher = QLineEdit(str(self.book[4]))
            self.lineEdit_publisher.setFixedWidth(200)
            self.lineEdit_publisher.returnPressed.connect(self.save)
            ...
            pushButton_save = QPushButton("Kaydet")
            pushButton_save.pressed.connect(self.save)
            pushButton_back = QPushButton("Geri Dön (İptal)")
            pushButton_back.pressed.connect(self.back)
            h_box_buttons.addStretch()
            h_box_buttons.addWidget(pushButton_back)
            h_box_buttons.addWidget(pushButton_save)
            h_box_buttons.addStretch()
    ...
    def save(self):
        ...
        try:
            bookManager = BookManager()
            bookManager.update_book(self.selected_book_id,
                                    name, author, number_of_pages, publisher)
        except Exception as exception:
            logger.error(exception)
            msgBox = QMessageBox()
            msgBox.setIcon(QMessageBox.Critical)
